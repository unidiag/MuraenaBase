...
func issueAccess(u *models.User) (string, string, error) {
	now := time.Now()
...
func issueRefresh(u *models.User, userAgent string, ttl time.Duration) (string, string, error) {
	now := time.Now()
	jti := strconv.FormatInt(now.UnixNano(), 36)
...
func makeUser(login, password string) error {
	...
	if count == 0 {
		...
		db.Create(&models.User{Login: login, Password: h, Status: 0})
		return nil
	}
